...
exports.createBootcamp = asyncHandler(async (req, res, next) => {
  ...
  // Check for published bootcamp - a publisher can only post one bootcamp
  const publishedBootcamp = await Bootcamp.findOne({ user: req.user.id });

  // If user is not an admin, they can only add 1 bootcamp
...
exports.updateBootcamp = asyncHandler(async (req, res, next) => {
  ...
  // Validate user of bootcamp is owner
...
exports.bootcampPhotoUpload = asyncHandler(async (req, res, next) => {
  ...
  if (bootcamp.user.toString() !== req.user.id && req.user.role !== 'admin') {
    return next(
      new ErrorResponse(`User ${req.params.id} is not allowed to update the bootcamp`, 401)
    );
  }
